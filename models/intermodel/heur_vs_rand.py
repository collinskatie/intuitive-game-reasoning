...
def get_model_pred(game, agent, early_stop=True, num_sims=100, other_agent="self", intermediate_board=None, seed=7, **kwargs):
    random.seed(seed)
    index = game['index']
    players = ['X', 'O']
    ...
    def simulate_one_game(starting_board, player_1, player_2, intermediate_board=None):
        ...
        if early_stop and plays >= num_plays and win_score(board, players[0], win_nums, for_win, constraints) == 0:
            ...
            # return (random.choices([0, 1, -1], [0.9, 0.05, 0.05], k=1)[0], plays)
    ...
    for i in range(num_sims):
        # EDIT: changed [katie] from agent,agent -> agent,other_agent to allow for agents to play other agents
        
        # if intermediate_board is not None:
