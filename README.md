...
- **`intermodel/`**: Inter-model comparison experiments
  ...
  - `heur_vs_rand.py`: Strategy comparison utilities

### Human and model data

**`human-data/`**: Contains data for each human experiment and processing scripts. Participants IDs for participants are removed (as they are only pseudoanonymized) and replaced with unique codes. 
...
- **`think-exp/`**: Judgements from participants from "just thinking" about the game (before any play). 
  ...
  - If you would like a processed version focused just on judgements, take a look at `analysis/final_processed_res/human_processed.json` (which has keys per game and all judgements). You can see example loading in `demo.ipyb`
...
- **`think-exp/`**: Model evaluation data (corresponding to human think-exp)
...
**Key files:**
- `cogsci_just_think_stimuli.csv`: Full set of stimuli for the 121 games (board sizes, winning conditions, labels)
- `parsable_stimuli_labels.csv`: Labeled game configurations
- `game2idx.json`: Mapping from games to indices
- `watch_game_stimuli.csv`: Stimuli for watch experiments (further subset of the 121)
- `updated_final_play_game.csv`: Finalized play experiment games
...
- `draw-combined.R`: draw request modeling
